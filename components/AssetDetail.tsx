...
import { ArrowLeft, ArrowUp, ArrowDown, Share2, MoreHorizontal, ChevronRight, LayoutGrid, FileText } from 'lucide-react';
...
const AssetDetail: React.FC<AssetDetailProps> = ({ asset, onBack }) => {
  ...
  // Mock chart data based on price
  const chartData = useMemo(() => {
    return Array.from({ length: 40 }, (_, i) => ({
      time: i.toString(),
      value: liveData.price * (1 + (Math.sin(i / 3) * 0.02) - (i * 0.0005))
    }));
  }, [asset.symbol]);
    ...
    <div className="animate-in slide-in-from-right duration-500 min-h-screen bg-[#050505] pb-32">
      ...
      <div className="px-4 py-4 space-y-1">
        ...
        <div className="flex items-end gap-3 mt-2">
          ...
          <div className={`flex items-center font-bold text-sm pb-1.5 ${isPositive ? 'text-green-500' : 'text-[#f97316]'}`}>
            ...
            <span>$0.68 ({asset.changePercent.toFixed(2)}%)</span>
      ...
      <div className="flex justify-between px-4 mb-10 overflow-x-auto scrollbar-hide">
        {['1D', '1W', '1M', '3M', '1Y', 'All'].map(t => (
          <button
            ...
            className={`px-4 py-2 rounded-full text-xs font-bold transition-all ${timeframe === t ? 'bg-emerald-500 text-black' : 'text-zinc-500 hover:text-white'}`}
          >
            {t}
          </button>
        ))}
      ...
      <div className="px-4 mb-10">
        ...
        <div className="flex gap-4">
          ...
          <button className="flex-1 bg-zinc-900/30 border border-white/10 text-zinc-300 font-extrabold py-3.5 rounded-full hover:bg-zinc-800 transition-all text-sm">
            Options
          </button>
        </div>
      </div>
      ...
      <div className="px-4 space-y-2 mb-10">
        {[
          ...
          <div key={item.symbol} className="flex items-center justify-between py-4 border-b border-white/5 group cursor-pointer">
            <div>
      ...
      <div className="px-4 pb-20">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-[10px] font-bold text-zinc-500">0 shown</span>
        </div>
