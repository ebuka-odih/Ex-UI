    ...
    <div className="px-4 py-6 space-y-6">
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-3">
          {['BTC', 'FTM', 'USDC', 'ETH'].map((coin) => (
            <div key={coin} className="bg-zinc-900/50 border border-white/5 p-4 rounded-xl">
              <h4 className="font-bold text-sm">{coin}</h4>
              <p className="text-[10px] text-zinc-500 font-bold mb-2">
                {coin === 'BTC' ? 'Bitcoin' : coin === 'FTM' ? 'Fantom' : coin === 'USDC' ? 'USD Coin' : 'Ethereum'}
              </p>
              <p className="text-green-500 text-[10px] font-bold">2 tracking</p>
            </div>
          ))}
        </div>
      </div>
